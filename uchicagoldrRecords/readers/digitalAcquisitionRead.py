def ReadAcquisitionRecord(digitalAcquisitionRecordPath):
	...
	#If we haven't hit the field data yet jump to the next line
        if currentField is None and ":" not in line:
            continue
	#If we find a line with a colon and if whats behind it is in the list of keys we are in a new field, grab the whole line even if it has more colons in it
        if ":" in line:
            if line.split(":")[0] in keyList:
                #Chomp trailing newlines
                fieldValue=""
                currentField=line.split(":")[0]
		#Manually strip the front of the line off, its a legitimate field (or a very strange outlier)
	...
	#Strip leading/trailing whitespace only from the start/end of values. Leave stuff in the middle for reconstruction later if need be.
            digitalAcquisitionRecordDict[value]=digitalAcquisitionRecordDict[value].strip()
        if value == "Donor Name" or value == "Donor Address" or value == "Donor Phone" or value == "Donor Email":
