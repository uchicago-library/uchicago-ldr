...
def ValidateBase(path,shouldBeRoot=None,shouldBeArk=None,shouldBeEAD=None,shouldBeAccNo=None):
    ...
    if len(listdir(join(shouldBeRoot,shouldBeArk,shouldBeEAD)))==1:
        observedAccNo=listdir(join(shouldBeRoot,shouldBeArk,shouldBeEAD))[0]
    else:
        return (False,observedRoot,observedArk,observedEAD,observedAccNo)
    ...
    for folder in ['admin','data']:
        if folder not in listdir(join(shouldBeRoot,shouldBeArk,shouldBeEAD,shouldBeAccNo)):
            return (False,observedRoot,observedArk,observedEAD,observedAccNo)
    if len(listdir(join(shouldBeRoot,shouldBeArk,shouldBeEAD,shouldBeAccNo)))!=2:
            return (False,observedRoot,observedArk,observedEAD,observedAccNo)

    return (True,observedRoot,observedArk,observedEAD,observedAccNo)
