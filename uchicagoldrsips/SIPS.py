...
def make_identifier(id_parts, files):
    ...
    pages = set([int(re_compile('_(\w{4}).*').search(basename(x.canonical_filepath)).group(1).lstrip('0'))
             for x in files if re_compile('_\w{4}.*').search(basename(x.canonical_filepath))])
    numpages = int(list(pages)[-1])
    return namedtuple("identifier", "directory, filename first_page numpages")  \
        (id_directory, id_filename, first_page_representation, numpages)

def make_providedcho(identifier, metadata):
    ...
    # hasparts = [str("dcterms:hasPart " + join(identifier.directory,
    #             identifier.filename,'_'+str(x))) \
    #             for x in range(identifier.numpages)]
    print(type(identifier.numpages))
    print(identifier.numpages)

    statements = ["dc:coverage \"Chicago\"", 
                  "dc:date \"{date}\"".format(date = metadata.date),
                  "edm:year \"{year}\"".format(year = metadata.date.split('-')[0]),
                  "dc:description \"{description}\"".format(dscription = metadata.description),
                  "dc:identifier \"{identifier}\"".format(identifier = identifier.filename),
                  "dc:language \"en\"",
                  "dc:rights <http://creativecommons.org/licenses/by-nc/4.0/>",
                  "dc:title \"{title}\"".format(title = metadata.title),
                  "dc:type \"text\"",
                  "edm:type \"TEXT\"",
